...
[Ładniejsze wykresy trójwymiarowe można wykonać korzystając z funkcji z pakietu 
`plot3D`.]
...
Auto[Auto$mpg > 30,]
Auto$mpg[Auto$mpg > 30]
Auto[Auto$mpg > 30,]
Auto[Auto$mpg > 30, "weight"]
```

### Prezentacja graficzna danych z ramki.

Wykresy rozproszenia (*scatter plots*).

```{r}
plot(Auto$cylinders, Auto$mpg)
plot(mpg ~ cylinders, data = Auto)
plot(mpg ~ as.factor(cylinders), data = Auto)
```
...
Macierz wykresów rozproszenia dla wielu par cech (zmiennych/kolumn).
