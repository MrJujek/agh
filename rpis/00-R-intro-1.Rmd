...
nich podstawowe funkcje statystyki opisowej: `mean()`, `median()`, `var()`,
`sd()`, `min()`, `max()`.
...
```
*Funkcja `runif()` (`?runif`) zwraca jedną lub więcej liczb wylosowanych według
jednostajnego rozkładu prawdopodobieństwa --- domyślnie na przedziale $[0, 1]$.*
...
Od wersji `R` 4.1.0 dostępny jest operator *potoku* [ang. *pipe*] `|>` 
pozwalający (formalnie) zastąpić zagnieżdżone wywołania funkcji łączeniem
tych wywołań w łańcuch, np.

```{r pipe}
# wywołania bez potoku
h(g(f(20)))
g(f(10, 3))

# są równonważne następującym konstrukcjom
20 |> f() |> g() |> h()
10 |> f(3) |> g()
```

Operator potoku jest alternatywą dla dostępnego również we wcześniejszych 
wersjach `R` podobnego operatora `%>%` zdefiniowanego w pakiecie `magrittr`
i wykorzystywanego w wielu innych pakietach.
